...
{\bfseries{Note}}\+: {\itshape If the C\+SR file is specified without also specifying a device private key, the Device Client will use Claim Certificate and Private key to generate new Certificate and Private Key while provisioning the device}

{\bfseries{Note}}\+: {\itshape If Fleet Provisioning feature is enabled, the Device Client will use the {\ttfamily thing-\/name} value as {\ttfamily client-\/id} {\bfseries{only}} for connecting to IoT Core the first time. Once your device connects to IoT Core, and is successfully provisioned, it will automatically be assigned a new {\ttfamily thing-\/name} according to your fleet provisioning template. This new {\ttfamily thing-\/name} will replace the old {\ttfamily client-\/id} and {\ttfamily thing-\/name} when the device subscribes to M\+Q\+TT topics for other enabled features.}
